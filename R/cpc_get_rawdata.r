...
cpc_get_rawdata <- function(begYr, begMo, begDay, endYr, endMo, endDay, usa = FALSE) {
  ...
  if (!usa) {
    ...
    urlHead  <- "https://ftp.cpc.ncep.noaa.gov/precip/CPC_UNI_PRCP/GAUGE_GLB/"
    fileHead <- "PRCP_CU_GAUGE_V1.0GLB_0.50deg.lnx."
  } else {
    urlHead  <- "https://ftp.cpc.ncep.noaa.gov/precip/CPC_UNI_PRCP/GAUGE_CONUS/"
    ...
    for (eachDay in 1:length(year_dates)) {
      ...
      if (!file.exists(outFile)) {
        ...
        while (fileError & max_tries < 10) {
          ...
          if (eachYr <= 2008) {
            fileStatus <- try(utils::download.file(url = fileUrl,
                                                   destfile = outFile,
                                                   quiet = TRUE),
                              silent = TRUE)
          } else {
